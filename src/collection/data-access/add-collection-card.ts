import type { Collection, Db, UpdateResult } from 'mongodb';
import type { CollectionCard } from '../model/collection-card.ts';

export interface AddCollectionCardParams {
	db: Db;
	collectionCard: CollectionCard;
}

export async function addCollectionCard({
	db,
	collectionCard,
}: AddCollectionCardParams): Promise<UpdateResult<CollectionCard>> {
	const cards: Collection<CollectionCard> =
		db.collection<CollectionCard>('cards');

	return await cards.updateOne(
		{ _id: collectionCard._id },
		{
			$inc: {
				'variants.normal': collectionCard.variants.normal ?? 0,
				'variants.reverse': collectionCard.variants.reverse ?? 0,
				'variants.firstEdition': collectionCard.variants.firstEdition ?? 0,
				'variants.holo': collectionCard.variants.holo ?? 0,
				'variants.wPromo': collectionCard.variants.wPromo ?? 0,
			},
		},
		{ upsert: true },
	);
}
