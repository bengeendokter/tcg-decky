...
export function convertQuantityToCharacter(quantity: number): string {
	...
	if (character === '') {
		return QUANTITY_CONVERSION_STRING.charAt(QUANTITY_CONVERSION_STRING.length - 1);
	}

	return character;
}

export function converLimitlessDeckToImportString(limitlessDeck: LimitlessDeck): string {
	const cardCodes: string[] = limitlessDeck.pokemon
		...
		.map((limitlessCard) => {
			...
			return [region, count, setAbbriviationLenght, localIdLength, setAbbriviation, localId].join(
				'',
			);
		});

	const energyCardCodes: string[] = limitlessDeck.energy.map((limitlessCard) => {
		const region: string = '0';
		const count: string = convertQuantityToCharacter(limitlessCard.quantity);
		const setAbbriviationLenght: string = convertQuantityToCharacter(
			limitlessCard.tcgOnline.length,
		...
		const setAbbriviation: string = limitlessCard.tcgOnline;
		const energyType: EnergyType = parseEnergyType(limitlessCard.name) ?? ENERGY_TYPES.FIRE;
