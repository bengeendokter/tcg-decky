...
export const COLOR_SCHEME: Readonly<{
...
type ContrastKey<OriginalType extends Contrast> =
	OriginalType extends `${infer ContrastLevel}-contrast`
		? Uppercase<`${ContrastLevel}_CONTRAST`>
		: Uppercase<OriginalType>;
type ContrastValue<Key extends ContrastKey<Contrast>> =
	Key extends `${infer ContrastLevel}_CONTRAST`
		? `${Lowercase<ContrastLevel>}-contrast`
		: Lowercase<Key>;
...
type ThemeClassKey<OriginalType extends ThemeClass> =
	OriginalType extends `${infer ColorScheme}-${infer ContrastLevel}-contrast`
		? Uppercase<`${ColorScheme}_${ContrastLevel}_CONTRAST`>
		: Uppercase<OriginalType>;
type ThemeClassValue<Key extends ThemeClassKey<ThemeClass>> =
	Key extends `${infer ColorScheme}_${infer ContrastLevel}_CONTRAST`
		? `${Lowercase<ColorScheme>}-${Lowercase<ContrastLevel>}-contrast`
		: Lowercase<Key>;

export const THEME_CLASS: Readonly<{
...
export function setMetaThemeColor(color?: string) {
	...
	if (!color) {
		const surfaceContainerColor = getComputedStyle(document.documentElement).getPropertyValue(
...
export function getColorSchemePreference(): ColorScheme {
	const storedPreferenceColorScheme: ColorScheme | typeof OS_PREFERENCE =
		getLocalStorageColorScheme();

	const osColorScheme: ColorScheme = window.matchMedia(MEDIA_QUERY.PREFERS_COLOR_SCHEME_DARK)
		.matches
		? COLOR_SCHEME.DARK
		: COLOR_SCHEME.LIGHT;
...
export function enableSystemColorSchemePreferenceListener() {
	...
	window.matchMedia(MEDIA_QUERY.PREFERS_COLOR_SCHEME_DARK).addEventListener('change', (event) => {
		const storedPreferenceColorScheme: string =
			localStorage.getItem(LOCAL_STORAGE.COLOR_SCHEME) ?? OS_PREFERENCE;
...
export function getContrastPreference(): Contrast {
	...
	const osContrast: Contrast = window.matchMedia(MEDIA_QUERY.PREFERS_CONTRAST_MORE).matches
...
export function enableSystemContrastPreferenceListener() {
	...
	window.matchMedia(MEDIA_QUERY.PREFERS_CONTRAST_MORE).addEventListener('change', (event) => {
		const storedPreferenceContrast: string =
			localStorage.getItem(LOCAL_STORAGE.CONTRAST) ?? OS_PREFERENCE;
